package com.liyaqing.soaptest;

import java.util.List;

/**
 * Created by liyaqing on 2017/2/9.
 */

public class MyObject {

    /**
     * statusFlag : success
     * statusMessage : SUCCESS
     * appointments : [{"fz":"","ghdzt":"2","yyfs":"2","yydm":"150621001","remark":"","czyhbh":"","zkdm":"","hzzcyhbh":"","zjdz":"","zkmc":"","ghdh":"16092109474885181","zjxgyhbh":"","id":"157","yyrq":"2016-09-22","scyydsj":"2016-09-21 09:47:48","qxyydsj":"2016-09-21 15:04:58","sfxz":"0","sfly":"-1","ghfy":"0","jzkh":"152723194911108123","pbbh":"15062100124150001201609010","ysgh":"0001","ksmc":"骨科","ksdm":"2415","scbz":"0","hyid":"15062100124150001201609010","yymc":"达旗人民医院","hzxm":"闫二女","yysjd":"0","tzqzt":"1","jzlx":"1","plzt":"0","lxdh":"13552634282","qxyyyhbh":"","zjxgrq":"","zjlx":"01","zjhm":"152723194911108123","sfpf":"0","yysdbz":"","ysxm":"传奇一哥"},{"fz":"","ghdzt":"0","yyfs":"2","yydm":"150621001","remark":"","czyhbh":"","zkdm":"","hzzcyhbh":"","zjdz":"","zkmc":"","ghdh":"16092710260844279","zjxgyhbh":"","id":"169","yyrq":"2016-10-02","scyydsj":"2016-09-27 10:26:08","qxyydsj":"","sfxz":"0","sfly":"-1","ghfy":"0","jzkh":"152723194911108123","pbbh":"15062100124150003201610021","ysgh":"0003","ksmc":"骨科","ksdm":"2415","scbz":"0","hyid":"15062100124150003201610021","yymc":"达旗人民医院","hzxm":"闫二女","yysjd":"1","tzqzt":"1","jzlx":"1","plzt":"0","lxdh":"13552634282","qxyyyhbh":"","zjxgrq":"","zjlx":"01","zjhm":"152723194911108123","sfpf":"0","yysdbz":"","ysxm":"传奇三哥"},{"fz":"","ghdzt":"0","yyfs":"2","yydm":"150621001","remark":"","czyhbh":"","zkdm":"","hzzcyhbh":"","zjdz":"","zkmc":"","ghdh":"16092710292681296","zjxgyhbh":"","id":"170","yyrq":"2016-10-02","scyydsj":"2016-09-27 10:29:26","qxyydsj":"","sfxz":"0","sfly":"-1","ghfy":"0","jzkh":"152723194911108123","pbbh":"15062100124150004201610021","ysgh":"0004","ksmc":"骨科","ksdm":"2415","scbz":"0","hyid":"15062100124150004201610021","yymc":"达旗人民医院","hzxm":"闫二女","yysjd":"0","tzqzt":"1","jzlx":"1","plzt":"0","lxdh":"13552634282","qxyyyhbh":"","zjxgrq":"","zjlx":"01","zjhm":"152723194911108123","sfpf":"0","yysdbz":"","ysxm":"传奇四哥"}]
     */

    private String statusFlag;
    private String statusMessage;
    /**
     * fz :
     * ghdzt : 2
     * yyfs : 2
     * yydm : 150621001
     * remark :
     * czyhbh :
     * zkdm :
     * hzzcyhbh :
     * zjdz :
     * zkmc :
     * ghdh : 16092109474885181
     * zjxgyhbh :
     * id : 157
     * yyrq : 2016-09-22
     * scyydsj : 2016-09-21 09:47:48
     * qxyydsj : 2016-09-21 15:04:58
     * sfxz : 0
     * sfly : -1
     * ghfy : 0
     * jzkh : 152723194911108123
     * pbbh : 15062100124150001201609010
     * ysgh : 0001
     * ksmc : 骨科
     * ksdm : 2415
     * scbz : 0
     * hyid : 15062100124150001201609010
     * yymc : 达旗人民医院
     * hzxm : 闫二女
     * yysjd : 0
     * tzqzt : 1
     * jzlx : 1
     * plzt : 0
     * lxdh : 13552634282
     * qxyyyhbh :
     * zjxgrq :
     * zjlx : 01
     * zjhm : 152723194911108123
     * sfpf : 0
     * yysdbz :
     * ysxm : 传奇一哥
     */

    private List<AppointmentsBean> appointments;

    public String getStatusFlag() {
        return statusFlag;
    }

    public void setStatusFlag(String statusFlag) {
        this.statusFlag = statusFlag;
    }

    public String getStatusMessage() {
        return statusMessage;
    }

    public void setStatusMessage(String statusMessage) {
        this.statusMessage = statusMessage;
    }

    public List<AppointmentsBean> getAppointments() {
        return appointments;
    }

    public void setAppointments(List<AppointmentsBean> appointments) {
        this.appointments = appointments;
    }

    public static class AppointmentsBean {
        private String fz;
        private String ghdzt;
        private String yyfs;
        private String yydm;
        private String remark;
        private String czyhbh;
        private String zkdm;
        private String hzzcyhbh;
        private String zjdz;
        private String zkmc;
        private String ghdh;
        private String zjxgyhbh;
        private String id;
        private String yyrq;
        private String scyydsj;
        private String qxyydsj;
        private String sfxz;
        private String sfly;
        private String ghfy;
        private String jzkh;
        private String pbbh;
        private String ysgh;
        private String ksmc;
        private String ksdm;
        private String scbz;
        private String hyid;
        private String yymc;
        private String hzxm;
        private String yysjd;
        private String tzqzt;
        private String jzlx;
        private String plzt;
        private String lxdh;
        private String qxyyyhbh;
        private String zjxgrq;
        private String zjlx;
        private String zjhm;
        private String sfpf;
        private String yysdbz;
        private String ysxm;

        public String getFz() {
            return fz;
        }

        public void setFz(String fz) {
            this.fz = fz;
        }

        public String getGhdzt() {
            return ghdzt;
        }

        public void setGhdzt(String ghdzt) {
            this.ghdzt = ghdzt;
        }

        public String getYyfs() {
            return yyfs;
        }

        public void setYyfs(String yyfs) {
            this.yyfs = yyfs;
        }

        public String getYydm() {
            return yydm;
        }

        public void setYydm(String yydm) {
            this.yydm = yydm;
        }

        public String getRemark() {
            return remark;
        }

        public void setRemark(String remark) {
            this.remark = remark;
        }

        public String getCzyhbh() {
            return czyhbh;
        }

        public void setCzyhbh(String czyhbh) {
            this.czyhbh = czyhbh;
        }

        public String getZkdm() {
            return zkdm;
        }

        public void setZkdm(String zkdm) {
            this.zkdm = zkdm;
        }

        public String getHzzcyhbh() {
            return hzzcyhbh;
        }

        public void setHzzcyhbh(String hzzcyhbh) {
            this.hzzcyhbh = hzzcyhbh;
        }

        public String getZjdz() {
            return zjdz;
        }

        public void setZjdz(String zjdz) {
            this.zjdz = zjdz;
        }

        public String getZkmc() {
            return zkmc;
        }

        public void setZkmc(String zkmc) {
            this.zkmc = zkmc;
        }

        public String getGhdh() {
            return ghdh;
        }

        public void setGhdh(String ghdh) {
            this.ghdh = ghdh;
        }

        public String getZjxgyhbh() {
            return zjxgyhbh;
        }

        public void setZjxgyhbh(String zjxgyhbh) {
            this.zjxgyhbh = zjxgyhbh;
        }

        public String getId() {
            return id;
        }

        public void setId(String id) {
            this.id = id;
        }

        public String getYyrq() {
            return yyrq;
        }

        public void setYyrq(String yyrq) {
            this.yyrq = yyrq;
        }

        public String getScyydsj() {
            return scyydsj;
        }

        public void setScyydsj(String scyydsj) {
            this.scyydsj = scyydsj;
        }

        public String getQxyydsj() {
            return qxyydsj;
        }

        public void setQxyydsj(String qxyydsj) {
            this.qxyydsj = qxyydsj;
        }

        public String getSfxz() {
            return sfxz;
        }

        public void setSfxz(String sfxz) {
            this.sfxz = sfxz;
        }

        public String getSfly() {
            return sfly;
        }

        public void setSfly(String sfly) {
            this.sfly = sfly;
        }

        public String getGhfy() {
            return ghfy;
        }

        public void setGhfy(String ghfy) {
            this.ghfy = ghfy;
        }

        public String getJzkh() {
            return jzkh;
        }

        public void setJzkh(String jzkh) {
            this.jzkh = jzkh;
        }

        public String getPbbh() {
            return pbbh;
        }

        public void setPbbh(String pbbh) {
            this.pbbh = pbbh;
        }

        public String getYsgh() {
            return ysgh;
        }

        public void setYsgh(String ysgh) {
            this.ysgh = ysgh;
        }

        public String getKsmc() {
            return ksmc;
        }

        public void setKsmc(String ksmc) {
            this.ksmc = ksmc;
        }

        public String getKsdm() {
            return ksdm;
        }

        public void setKsdm(String ksdm) {
            this.ksdm = ksdm;
        }

        public String getScbz() {
            return scbz;
        }

        public void setScbz(String scbz) {
            this.scbz = scbz;
        }

        public String getHyid() {
            return hyid;
        }

        public void setHyid(String hyid) {
            this.hyid = hyid;
        }

        public String getYymc() {
            return yymc;
        }

        public void setYymc(String yymc) {
            this.yymc = yymc;
        }

        public String getHzxm() {
            return hzxm;
        }

        public void setHzxm(String hzxm) {
            this.hzxm = hzxm;
        }

        public String getYysjd() {
            return yysjd;
        }

        public void setYysjd(String yysjd) {
            this.yysjd = yysjd;
        }

        public String getTzqzt() {
            return tzqzt;
        }

        public void setTzqzt(String tzqzt) {
            this.tzqzt = tzqzt;
        }

        public String getJzlx() {
            return jzlx;
        }

        public void setJzlx(String jzlx) {
            this.jzlx = jzlx;
        }

        public String getPlzt() {
            return plzt;
        }

        public void setPlzt(String plzt) {
            this.plzt = plzt;
        }

        public String getLxdh() {
            return lxdh;
        }

        public void setLxdh(String lxdh) {
            this.lxdh = lxdh;
        }

        public String getQxyyyhbh() {
            return qxyyyhbh;
        }

        public void setQxyyyhbh(String qxyyyhbh) {
            this.qxyyyhbh = qxyyyhbh;
        }

        public String getZjxgrq() {
            return zjxgrq;
        }

        public void setZjxgrq(String zjxgrq) {
            this.zjxgrq = zjxgrq;
        }

        public String getZjlx() {
            return zjlx;
        }

        public void setZjlx(String zjlx) {
            this.zjlx = zjlx;
        }

        public String getZjhm() {
            return zjhm;
        }

        public void setZjhm(String zjhm) {
            this.zjhm = zjhm;
        }

        public String getSfpf() {
            return sfpf;
        }

        public void setSfpf(String sfpf) {
            this.sfpf = sfpf;
        }

        public String getYysdbz() {
            return yysdbz;
        }

        public void setYysdbz(String yysdbz) {
            this.yysdbz = yysdbz;
        }

        public String getYsxm() {
            return ysxm;
        }

        public void setYsxm(String ysxm) {
            this.ysxm = ysxm;
        }
    }
}
